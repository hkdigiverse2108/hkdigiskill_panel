...
class CreatePartnersController extends GetxController {
  var isLoading = false.obs;

  final apiService = ApiService(baseUrl: ApiConstants.baseUrl);
  final storageService = AdminStorageService();

  final createPartnersFormKey = GlobalKey<FormState>();
  ...
  Future<void> createPartners() async {
    try {
      ...
      if (!createPartnersFormKey.currentState!.validate()) {
        return;
      }

      final response = await apiService.post(
        path: ApiConstants.partnerCreate,
      ...
      log(e.toString());
      AdminLoaders.errorSnackBar(title: "Create Partners error", message: "$e");
    } finally {
